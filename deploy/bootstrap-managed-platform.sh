...
ensure_state_directory() {
  ...
  if [ -L "$target" ] || { [ -e "$target" ] && [ ! -d "$target" ]; }; then
    fail "managed state directory is invalid: $target"
  fi
  if [ ! -d "$target" ]; then
    install -d -o root -g root -m 0700 "$target"
  fi
  chown root:root "$target"
  setfacl -m g::---,o::--- "$target"
}
...
repair_control_database_access
ensure_state_directory "$STATE_ROOT/tenants"
ensure_state_directory "$STATE_ROOT/backups"
ensure_state_directory "$STATE_ROOT/tenant-credentials"
ensure_state_directory "$STATE_ROOT/tenant-runtime"
