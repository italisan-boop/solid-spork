...
class PlatformConsoleHtmlTests(unittest.TestCase):
    def test_telegram_sdk_loads_before_console_initialization(self):
        ...
        self.assertIn("/provision`, {method:'POST'", page)
        self.assertIn("/activate`, {method:'POST'", page)
        ...
        self.assertIn("/deployments`, {method:'POST'", page)
        self.assertIn("Введите secret значение или используйте очистку proxy", page)
        self.assertIn("data-redeploy", page)
